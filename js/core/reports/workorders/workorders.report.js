...
export const generateWorkOrderReport = async (order) => {
    ...
    const paymentStatusLabel = {
        ...
        'PAID': 'PAGADO',
        'PARTIAL': 'PARCIAL'
    }[order.paymentStatus?.toUpperCase()] ?? order.paymentStatus?.toUpperCase() ?? '—';
    ...
    doc.setTextColor(255, 200, 200);
    doc.text(
        `${order.vehicleInfo?.brand ?? '—'} ${order.vehicleInfo?.model ?? ''} · ${order.vehicleInfo?.year ?? '—'}`,
        pageW - margin,
        28,
        { align: 'right' }
    );
    ...
    const orderStatusLabel = `ORDEN: ${order.status?.toUpperCase() || '—'}`;
    const paymentBadgeLabel = `PAGO: ${paymentStatusLabel}`;
    ...
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'bold');
    doc.text(orderStatusLabel, pageW - margin - b1W / 2, y, { align: 'center' });
    ...
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'bold');
    doc.text(paymentBadgeLabel, pageW - margin - b1W - b2W - 4 + b2W / 2, y, { align: 'center' });
    ...
    // ═══════════════════════════════════════
    // CLIENTE ARRIBA — Vehículo | Orden abajo
    // ═══════════════════════════════════════
    const infoGap = 6;
    const colW = (contentW - infoGap) / 2;
    const col2X = margin + colW + infoGap;

    const customerRows = [
        { label: 'Cliente', value: order.customer?.fullName },
        { label: 'DUI', value: order.customer?.dui },
        { label: 'Teléfono', value: order.customer?.personalPhone },
        { label: 'Estado', value: order.customer?.status === 'ACTIVE' ? 'Activo' : order.customer?.status }
    ];

    const vehicleRows = [
        { label: 'VIN', value: order.vehicleInfo?.vin },
        { label: 'Marca / Modelo', value: `${order.vehicleInfo?.brand ?? '—'} ${order.vehicleInfo?.model ?? ''}`.trim() },
    ...
    const customerH = customerRows.length * 9 + 16;
    const colsH = Math.max(
        vehicleRows.length * 9 + 16,
        orderRows.length * 9 + 16
    );

    y = checkPageBreak(doc, y, customerH + colsH + 22, pageH, margin, pageW);
    y = sectionHeader(doc, y, contentW, margin, pageW, 'INFORMACIÓN DE LA ORDEN');
    y += 4;

    // Cliente arriba ocupando todo el ancho
    drawInfoBox(doc, customerRows, margin, y, contentW, 'CLIENTE');

    y += customerH + 8;

    // Vehículo y orden abajo en dos columnas
